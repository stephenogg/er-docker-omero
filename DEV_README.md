...
8. Open a web browser and enter the following address to connect to your local OMERO : http://localhost:4080/ and connect with the root credentials

> **Note:** It can take a while since the server is ready. During that time, you may have login errors.

You have now an OMERO server that you can use to try out stuff. This server, build with the standard `docker-compose.yaml`, doesn't allow you to modify plugin's version nor to develop and test your own code (i.e. it is just a running instance of OMERO). Let's do the others steps to make it configurable.

## Step 2 : Specifying a version of a plugin
This step shows how it is possible to create an omero-server with a specific version of a plugin (ex: omero-figure 7.0.0) running on it.
...
USER omero-web
````
- Line 1 : Indicates which docker image of omero-web you are using as reference. The version number can be changed according to the version you want to run AND the version available on docker-hub. For simplicity, we are using the keyword `latest`.
- Line 2 : Indicates who runs the following command ; it has to be **root** if to make any modification.
...
> **Note:** Remove `omero` from the config line before pasting the it in the `01-default-webapps.omero` file
...
2. Add the following lines (3 to 5) to the Dockerfile, right-after adding the config file.
...
- Line 3 : Copying the content of the local omero-figure to the `/home/figure/src` folder in your container. This is necessary if you would like to test the code you'll develop under `omero-figure`.
...
4. In order to dynamically see changes in your docker when you are coding under omero-figure, you need to add a `volume` that synchronizes local file with your docker.
